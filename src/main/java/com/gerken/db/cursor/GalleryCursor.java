...
/**
 * Forward-only, read-only cursor over a {@link Gallery} result set.
 *
...
public class GalleryCursor {
    ...
    /**
     * Creates a GalleryCursor wrapping the given statement and result set.
     *
     * @param ps the prepared statement that produced {@code rs}
     * @param rs the result set to iterate
     */
    private GalleryCursor(PreparedStatement ps, ResultSet rs) {
        this.ps = ps;
        this.rs = rs;
    }

    /**
     * Opens a cursor over all Gallery rows that match the given SQL clauses.
     *
     * <p>The cursor reads all columns ({@code id, poster, name, url, attrs, images,
     * pages, pagesSeen, quality, added, state}) from the {@code Gallery} table.
     * The optional {@code clauses} string is appended directly to the base SELECT;
     * it may contain {@code WHERE}, {@code ORDER BY}, {@code LIMIT}, or any other
     * trailing SQL fragment.</p>
     *
     ...
     */
    public static GalleryCursor select(String clauses) throws SQLException {
        Connection conn = PersistCrawlerIf.getConnection();
        String sql = "SELECT id, poster, name, url, attrs, images, pages, pagesSeen, quality, added, state FROM Gallery";
        if (clauses != null && !clauses.isBlank()) sql += " " + clauses;
        PreparedStatement ps = conn.prepareStatement(sql);
        return new GalleryCursor(ps, ps.executeQuery());
    }

    /**
     * Returns the number of Gallery rows that match the given SQL clauses.
     *
    ...
    /**
     * Advances the cursor and returns the next {@link Gallery}.
     *
     * <p>Integer columns ({@code images}, {@code pages}, {@code pagesSeen}) are read
     * with {@code wasNull()} to correctly handle SQL {@code NULL} values. When the
     * result set is exhausted, {@link #close()} is called automatically and
     * {@code null} is returned.</p>
     *
     * @return the next Gallery, or {@code null} if there are no more rows
     * @throws SQLException if a database error occurs
     */
    public Gallery next() throws SQLException {
        if (!rs.next()) {
            close();
            return null;
        }
        Gallery bean = new Gallery(rs.getString("id"));
        bean.setPoster(rs.getString("poster"));
        bean.setName(rs.getString("name"));
        bean.setUrl(rs.getString("url"));
        bean.setAttrs(rs.getString("attrs"));
        int images    = rs.getInt("images");    bean.setImages(rs.wasNull()    ? null : images);
        int pages     = rs.getInt("pages");     bean.setPages(rs.wasNull()     ? null : pages);
        int pagesSeen = rs.getInt("pagesSeen"); bean.setPagesSeen(rs.wasNull() ? null : pagesSeen);
        bean.setQuality(rs.getString("quality"));
     ...
     *
     * @return an array of all remaining Gallery rows; never {@code null}
     * @throws SQLException if a database error occurs
     */
    public Gallery[] getAll() throws SQLException {
        List<Gallery> list = new ArrayList<>();
        Gallery row;
        while ((row = next()) != null) list.add(row);
        return list.toArray(new Gallery[0]);
    }
