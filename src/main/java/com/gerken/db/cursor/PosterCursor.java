...
/**
 * Forward-only, read-only cursor over a {@link Poster} result set.
 *
...
public class PosterCursor {
    ...
    /**
     * Creates a PosterCursor wrapping the given statement and result set.
     *
     * @param ps the prepared statement that produced {@code rs}
     * @param rs the result set to iterate
     */
    private PosterCursor(PreparedStatement ps, ResultSet rs) {
        this.ps = ps;
        this.rs = rs;
    }

    /**
     * Opens a cursor over all Poster rows that match the given SQL clauses.
     *
     * <p>The cursor reads all columns ({@code name, url, color}) from the
     * {@code Poster} table. The optional {@code clauses} string is appended
     ...
     */
    public static PosterCursor select(String clauses) throws SQLException {
        Connection conn = PersistCrawlerIf.getConnection();
        String sql = "SELECT name, url, color FROM Poster";
        if (clauses != null && !clauses.isBlank()) sql += " " + clauses;
        PreparedStatement ps = conn.prepareStatement(sql);
        return new PosterCursor(ps, ps.executeQuery());
    }

    /**
     * Returns the number of Poster rows that match the given SQL clauses.
     *
    ...
    public static int count(String clauses) throws SQLException {
        Connection conn = PersistCrawlerIf.getConnection();
        String sql = "SELECT COUNT(*) FROM Poster";
    ...
    /**
     * Advances the cursor and returns the next {@link Poster}.
     *
     * <p>When the result set is exhausted, {@link #close()} is called automatically
     * and {@code null} is returned.</p>
     *
     * @return the next Poster, or {@code null} if there are no more rows
     * @throws SQLException if a database error occurs
     */
    public Poster next() throws SQLException {
        if (!rs.next()) {
            close();
            return null;
        }
        Poster bean = new Poster(rs.getString("name"));
     ...
     *
     * @return an array of all remaining Poster rows; never {@code null}
     * @throws SQLException if a database error occurs
     */
    public Poster[] getAll() throws SQLException {
        List<Poster> list = new ArrayList<>();
        Poster row;
        while ((row = next()) != null) list.add(row);
        return list.toArray(new Poster[0]);
    }
